...
Trains on Fashion-MNIST dataset with class-specific feedback vectors.
...
def load_fashion_mnist_dataset() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load Fashion-MNIST dataset using python-mnist."""
    try:
    ...
    # Fashion-MNIST path: data/mnist/fashion-mnist/
    data_path = Path(__file__).parent / "data" / "mnist" / "fashion-mnist"
    if not data_path.exists():
        # Try legacy path
        data_path = Path(__file__).parent / "data" / "fashion_mnist_data"
    
    if not data_path.exists():
        raise FileNotFoundError(
            f"Fashion-MNIST dataset not found at {data_path}. "
    ...
    print(f"Loaded Fashion-MNIST dataset:")
...
def add_gaussian_noise(images: np.ndarray, noise_std: float, seed: Optional[int] = None) -> np.ndarray:
    ...
    Args:
        images: Input images, shape (batch, height, width) or (batch, features)
        noise_std: Standard deviation of Gaussian noise
        seed: Optional random seed for reproducibility
        
    Returns:
        Noisy images with same shape as input, clipped to [0, 1]
    """
    if noise_std <= 0:
        return images
...
def train_epoch(
    ...
    for batch_idx in pbar:
        ...
        if noise_std > 0:
            batch_images = add_gaussian_noise(batch_images, noise_std, seed=None)
...
def main():
    ...
    print("=" * 60)
    print("HyperColumn FDL GPU Training - Fashion-MNIST")
    print("=" * 60)
    print()
