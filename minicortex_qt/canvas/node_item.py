...
from ..panels.palette import _generate_neon_color
...
def _generate_category_color(category: str) -> QColor:
    ...
    match the branch colors in the palette tree.
    
    The category is a branch path like "Input/Noise", but we use only the
    last part ("Noise") to match the palette's color generation.
    """
    if not category:
        return QColor()
    
    # Extract the last part of the branch path to match palette behavior
    # e.g., "Input/Noise" -> "Noise"
    branch_name = category.split("/")[-1] if "/" in category else category
    
    # Use the same color generation as the palette
    hex_color = _generate_neon_color(branch_name)
...
class NodeItem(QGraphicsItem):
    ...
    def _layout(self) -> None:
        ...
        if n_port_rows > 0:
            ...
            for i, port in enumerate(self.input_ports):
                port.setPos(0, io_top + i * PORT_ROW_HEIGHT + PORT_ROW_HEIGHT / 2)
            for i, port in enumerate(self.output_ports):
                port.setPos(self._width, io_top + i * PORT_ROW_HEIGHT + PORT_ROW_HEIGHT / 2)
    ...
    def set_error_state(self, has_error: bool) -> None:
        """Set the error state of the node (red border when True)."""
