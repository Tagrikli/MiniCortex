...
Trains on MNIST (digits) dataset with class-specific feedback vectors.
...
def load_mnist_dataset() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load MNIST (digits) dataset using python-mnist."""
    try:
    ...
    # MNIST path: data/mnist/
    data_path = Path(__file__).parent / "data" / "mnist"
...
def main():
    ...
    print("=" * 60)
    print("HyperColumn FDL GPU Training - MNIST (Digits)")
    print("=" * 60)
    print()
